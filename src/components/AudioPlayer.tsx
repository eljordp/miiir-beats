...
export default function AudioPlayer({ beat, isPlaying, onPlayPause }: AudioPlayerProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-surface/95 backdrop-blur-md border-t border-border safe-bottom">
      {/* Mobile progress bar at top of player */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center gap-3 sm:gap-6">
        ...
        <button
          onClick={onPlayPause}
          className="w-10 h-10 rounded-full bg-accent text-background flex items-center justify-center flex-shrink-0 hover:bg-accent-dim active:scale-95 transition-all"
        >
          {isPlaying ? (
        ...
        <span className="sm:hidden text-[10px] text-foreground/40 tabular-nums">{beat.duration}</span>
